...
func GetArticle(ctx context.Context, articleID int64) (*Article, error) {
	...
	if err != nil {
		log.Printf("ctx %v GetArticle articleid %v err %v", ctx, articleID, err)
		return nil, err
	}
...
func GetBatchArticle(ctx context.Context, articleIDs []int64) (map[int64]*Article, error) {
	...
	if err != nil {
		log.Printf("ctx %v GetBatchArticle articleid %v err %v", ctx, articleIDs, err)
		return nil, err
	}
...
func GetTopic(ctx context.Context, topicID int64) (*Topic, error) {
	...
	if err != nil {
		log.Printf("ctx %v GetTopic topicid %v err %v", ctx, topicID, err)
		return nil, err
	}
...
func GetBatchTopic(ctx context.Context, topicIDs []int64) (map[int64]*Topic, error) {
	...
	if err != nil {
		log.Printf("ctx %v GetBatchTopic topicids %v err %v", ctx, topicIDs, err)
		return nil, err
	}
...
func PublishArticle(ctx context.Context, articleID, uid, topicID int64, content string, vType int32) error {
	...
	if err != nil {
		log.Printf("ctx %v PublishArticle articleid %v err %v", ctx, articleID, err)
	}
	return err
}
...
func DeleteArticle(ctx context.Context, articleID int64) error {
	...
	if err != nil {
		log.Printf("ctx %v DeleteArticle articleid %v err %v", ctx, articleID, err)
	}
	return err
}
...
func PushFollowFeed(ctx context.Context, uid, articleID int64, uids []int64) error {
	...
	defer stream.Close()
	if err != nil {
		log.Printf("ctx %v PushFollowFeed uid %v articleid %v uids %v err %v", ctx, uids, articleID, uids, err)
		return err
	}
	for i := 0; i < len(uids); i += constant.BatchSize {
		...
		if err != nil {
			log.Printf("ctx %v PushFollowFeed streamsend uid %v articleid %v uids %v err %v", ctx, uids, articleID, uids, err)
			return err
		}
	}
	return nil
